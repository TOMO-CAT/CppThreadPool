...
class SyncTaskQueue {
 public:
  ...
  ~SyncTaskQueue();
  void addTask(ThreadPool::WorkerFunc* func, void* sync_task_ptr);
  void wait();
 ...
 private:
  ...
  static ThreadPool::WorkerFunc workerFuncWrapper;
};
...
inline void SyncTaskQueue::addTask(ThreadPool::WorkerFunc* func, void* arg) {
  sync_task_num_++;
